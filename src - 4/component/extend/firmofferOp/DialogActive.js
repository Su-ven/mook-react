...
var form = React.createClass({
    ...
    handleSend(e){
        ...
        this.props.form.validateFields((errors, values) => {
            ...
            fluxAccount.actions.sendActive(params, (result) => {
    ...
    render() {
        ...
        const fieldProps = {
            ...
            signBank: getFieldProps('signBank', {
                rules: [
                    // {required: true, message: '请选择签约银行'},
                ],
            }),
            signCard: getFieldProps('signCard', {
                rules: [
                    // {required: true, message: '预签约卡号不能为空'},
                    {validator: ValidResult.ynBankCard},
                ]
            }),
            ...
            activeType: getFieldProps('activeType', {
                initialValue: "1",
                rules: [
                    {required: true, message: '请选择激活类型'},
                ],
            }),
        };
        ...
        let title = "申请激活【" + this.props.plat.name + "】实盘账号";
        return <Modal
            ...
            <Form form={this.props.form}>
                <Row>
                    <Col span="12">
                        <FormItem {...formItemLayout}
                            label="实盘账号：">
                            <Input placeholder="请填写实盘账号" utoComplete="off" {...fieldProps["accountNumber"]} disabled/>
                        </FormItem>
                    </Col>
                    <Col span="12">
                        <FormItem {...formItemLayout}
                ...
                <Row>
                    <Col span="12">
                        <FormItem {...formItemLayout}
                            label="签约银行：">
                            <Select placeholder="请选择签约银行" autoComplete="off" {...fieldProps["signBank"]} >
                                {Config.banks.map(d => <Option key={d.value}>{d.text}</Option>)}
                            </Select>
                        </FormItem>
                    </Col>
                    <Col span="12">
                        <FormItem {...formItemLayout}
                            label="预签约卡号：">
                            <Input placeholder="请填写签约卡号" autoComplete="off" {...fieldProps["signCard"]} />
                        </FormItem>
                    </Col>
                </Row>                
                <Row>
                ...
                <Row>
                    <Col span="24">
                        <FormItem labelCol={{span: 4}} wrapperCol={{span: 19}}
                                  label="激活选项">
                            <RadioGroup {...fieldProps["activeType"]}>
                                <RadioButton value="1">首次激活</RadioButton>
                                <RadioButton value="2">二次激活</RadioButton>
                            </RadioGroup>
                        </FormItem>
                    </Col>                
                </Row>
